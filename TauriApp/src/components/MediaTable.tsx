...
type MediaTableProps = {
  ...
  setTableData: React.Dispatch<
    React.SetStateAction<SelectedMediaElement[] | null>
  >;
...
const MediaTable: React.FC<MediaTableProps> = ({
  ...
  const getData = async () => {
    const dataKey =
      projOrTimelineSelected === "project"
        ? dataFetchParameters.projectKey
        : dataFetchParameters.timelineKey;
    try {
  ...
  useEffect(() => {
    async function fetchData() {
      if (
        showDataTable &&
        (showDataTableProp === undefined || showDataTableProp === false)
      ) {
